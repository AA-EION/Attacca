...
pub fn abrir_repositorio(estado: &Estado, raiz: String) -> R<AperturaRepositorio> {
    let ruta = PathBuf::from(&raiz);
    estado.abrir(ruta.clone())?;
    apertura(&ruta)
}

pub fn crear_repositorio(estado: &Estado, raiz: String) -> R<AperturaRepositorio> {
    let ruta = PathBuf::from(&raiz);
...
pub fn limpiar_temporales(estado: &Estado) -> R<usize> {
...
pub fn identidad(estado: &Estado) -> Identidad {
    estado.identidad()
}

pub fn set_identidad(estado: &Estado, identidad: Identidad) {
...
pub fn comprobar_reloj(estado: &Estado) -> EstadoReloj {
    let s = clock::check_sync(clock::DEFAULT_SOURCES, std::time::Duration::from_secs(4));
    estado.set_reloj(s);
    proyectar_reloj(s)
}

pub fn reloj_conocido(estado: &Estado) -> EstadoReloj {
...
pub fn catalogo(estado: &Estado) -> R<Catalogo> {
    let repo = estado.repositorio()?;
...
pub fn reconstruir_indice(estado: &Estado) -> R<usize> {
    let repo = estado.repositorio()?;
...
pub fn crear_proyecto(estado: &Estado, datos: DatosProyecto) -> R<VistaProyecto> {
    let repo = estado.repositorio()?;
...
pub fn ver_proyecto(estado: &Estado, uid: String) -> R<VistaProyecto> {
    let m = cargar(estado, &uid)?;
    Ok(estado::vista_de(&m))
}

pub fn cambiar_titulo(estado: &Estado, uid: String, titulo: String) -> R<VistaProyecto> {
    let repo = estado.repositorio()?;
...
pub fn cambiar_estado(estado: &Estado, uid: String, nuevo: String) -> R<VistaProyecto> {
    let repo = estado.repositorio()?;
    let mut m = cargar(estado, &uid)?;
...
pub fn registrar_audio(estado: &Estado, uid: String, datos: DatosAudio) -> R<VistaProyecto> {
    let mut m = cargar(estado, &uid)?;
...
pub fn crear_sesion(estado: &Estado, uid: String, daw: String, etapa: String) -> R<CarpetaSesion> {
    let m = cargar(estado, &uid)?;
...
pub fn listar_carpeta(estado: &Estado, uid: String, relativa: String) -> R<Vec<Entrada>> {
    let m = cargar(estado, &uid)?;
    let raiz = m.root();
...
pub fn abrir_en_explorador(estado: &Estado, uid: String, relativa: String) -> R<()> {
    let m = cargar(estado, &uid)?;
    let destino = if relativa.is_empty() {
        m.root().to_path_buf()
    } else {
...
pub fn validar(estado: &Estado, uid: String) -> R<Conformidad> {
    let m = cargar(estado, &uid)?;
...
pub fn verificar_integridad(estado: &Estado, uid: String) -> R<ResultadoIntegridad> {
    let m = cargar(estado, &uid)?;
...
pub fn generar_integridad(estado: &Estado, uid: String) -> R<usize> {
    let m = cargar(estado, &uid)?;
...
pub fn crear_release(estado: &Estado, datos: DatosRelease) -> R<String> {
    let repo = estado.repositorio()?;
    let identidad = estado.identidad();
...
pub fn release_listo(estado: &Estado, release_uid: String) -> R<Vec<String>> {
    let repo = estado.repositorio()?;
...
pub fn emitir_envio(estado: &Estado, uid: String, datos: DatosEnvio) -> R<ResultadoEmision> {
    let repo = estado.repositorio()?;
    let mut m = cargar(estado, &uid)?;
    let identidad = estado.identidad();
...
pub fn cancelar_operacion(estado: &Estado) {
    estado.cancelar();
}
...
pub fn verificar_paquete(estado: &Estado, datos: DatosRecepcion) -> R<ResultadoVerificacion> {
    let repo = estado.repositorio()?;
...
pub fn ingerir_paquete(estado: &Estado, datos: DatosIngesta) -> R<ResultadoIngesta> {
    let repo = estado.repositorio()?;
    let ctx = contexto(estado, &datos.emisor, datos.condiciones_aceptables);
    estado.reiniciar_cancelacion();
    let bandera = estado.bandera_cancelacion();
...
pub fn paquetes_en_cuarentena(estado: &Estado) -> R<Vec<String>> {
    let repo = estado.repositorio()?;
    Ok(repo
...
pub fn reclamar_retorno(estado: &Estado, uid: String) -> R<String> {
    let repo = estado.repositorio()?;
    let m = cargar(estado, &uid)?;
...
pub fn recuperar_custodia(estado: &Estado, uid: String) -> R<VistaProyecto> {
    let repo = estado.repositorio()?;
    let mut m = cargar(estado, &uid)?;
...
pub fn derivar_sobre_cedido(estado: &Estado, uid: String) -> R<ResultadoDerivacion> {
    let repo = estado.repositorio()?;
    let m = cargar(estado, &uid)?;
...
pub fn registro(estado: &Estado, ultimas: usize) -> R<Vec<EntradaRegistro>> {
    let repo = estado.repositorio()?;
...
pub fn verificar_registro(estado: &Estado) -> R<EstadoRegistro> {
    let repo = estado.repositorio()?;
...
fn cargar(estado: &Estado, uid: &str) -> R<ProjectManifest> {
    let repo = estado.repositorio()?;
